...
SECTION_MENTION_CAPS: dict[str, int] = {
    "projects": 2,
    "work_experience": 3,
    "education": 1,
}
...
def _extract_candidate_years(work_entries: list[str]) -> tuple[float, dict[str, Any]]:
    ...
    if explicit:
        try:
            explicit_years = max(float(v) for v in explicit)
        except Exception:
            explicit_years = None
    ...
    if months_total > 0:
        ...
        return years, {
            ...
            "explicit_years_hint": explicit_years,
            "method": "date_ranges",
        }
    ...
    if explicit_years is not None:
        return explicit_years, {
            ...
            "excluded_as_internship": excluded,
            "ranges_detected": [],
            "months_total": int(round(explicit_years * 12)),
            "explicit_years_hint": explicit_years,
...
def _normalize_skill_token(value: str) -> str:
    normalized = "".join(ch for ch in value.lower().strip() if ch.isalnum() or ch in {"+", ".", "#"})
    # Keep internal dots for tokens like node.js, but avoid punctuation-only suffix noise (e.g. aws.).
...
def _entry_to_text(entry: object) -> str:
    if isinstance(entry, dict):
        parts: list[str] = []
        # Accept multiple possible key shapes returned by LLM parsers.
        for key in (
            "name",
            "title",
            "role",
...
def _is_duplicate_evidence(candidate: str, seen: set[str]) -> bool:
    ...
    if fp in seen:
        return True
    # Treat near-duplicate snippets as the same evidence when one line contains the other.
...
def _extract_skills_window_lines(raw_text: str) -> list[str]:
    ...
    for line in lines:
        if ":" in line:
            ...
            if inline_section:
                in_skills = inline_section == "skills"
                if in_skills:
                    payload = right.strip()
...
def _preferred_section_lines(buckets: dict[str, list[str]], section: str) -> list[str]:
    raw_key = f"{section}_raw_section"
    raw_lines = [str(v or "").strip() for v in buckets.get(raw_key, []) if str(v or "").strip()]
    if raw_lines:
        return raw_lines
    return [str(v or "").strip() for v in buckets.get(section, []) if str(v or "").strip()]
...
def _collect_skill_evidence(
    ...
    for section in sections:
        section_lines = _preferred_section_lines(buckets, section)

        for text_value in section_lines:
            if not text_value:
                continue
            if _is_duplicate_evidence(text_value, seen):
                continue
            if not _line_matches_variants(text_value, variants):
                continue

            fp = _evidence_fingerprint(text_value)
            if fp:
                seen.add(fp)
...
def _count_mentions_in_sections(
    ...
    variants: set[str],
    sections: tuple[str, ...],
) -> tuple[int, set[str], dict[str, int], dict[str, int], int]:
    raw_section_hits: dict[str, int] = {}
    capped_section_hits: dict[str, int] = {}
    ...
    for section in sections:
        ...
        for text_value in section_lines:
            ...
            # Count one mention event per unique evidence unit, not per token repeat.
            section_hits += 1
            matched_aliases.update(aliases_in_line)

        raw_section_hits[section] = section_hits
        cap = SECTION_MENTION_CAPS.get(section, section_hits)
        capped_section_hits[section] = min(section_hits, cap)

    mentions = sum(capped_section_hits.values())
    return mentions, matched_aliases, raw_section_hits, capped_section_hits, duplicate_drops
...
def _skill_component(
    ...
    for skill, is_primary in all_skills:
        ...
        mentions, matched_aliases, raw_section_mentions, capped_section_mentions, duplicate_drops = _count_mentions_in_sections(
            buckets,
            variants,
        ...
        details.append(
            {
                ...
                "bonus": bonus,
                "mentions": mentions,
                "raw_section_mentions": raw_section_mentions,
                "capped_section_mentions": capped_section_mentions,
                "duplicate_evidence_dropped": duplicate_drops,
                "score": score,
